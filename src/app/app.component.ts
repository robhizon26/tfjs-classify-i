...
export class AppComponent implements OnInit {
    ...
    if (plgVl.value) {
      ...
      if (this.savedModel) {
        ...
        const availableModels = {
          any: [modelurl, { size: this.savedModel.info.dim, type: this.savedModel.info.format, labels: this.labels }],
        };
  ...
  private async isModelPathNotLegit(fullmodelpath) {
    ...
    try {
      ...
      return { title: 'File not found', message: 'There was an error retreiving ' + fullmodelpath + 'labels.txt' }
    }
    try {
      textvalue = await this.httpClient.get(fullmodelpath + 'model.json', { responseType: 'text' }).toPromise();
    } catch {
      return { title: 'File not found', message: 'There was an error retreiving ' + fullmodelpath + 'model.json' }
    }
    return null;
  }
  ...
  private resizeByCanvas(img, width, height, padding) {
    let newheight = height;
    let newheightneglee = 0;
    //compute for desired video capture width with % padding on each side
    const paddedWidth = width * (1 - padding);
    const paddedHeight = newheight * (1 - padding);
    let VIDEO_PIXELS = paddedWidth < paddedHeight ? paddedWidth : paddedHeight;//224;
    const centerWidth = width / 2;
    const centerHeight = (height / 2) - newheightneglee;
    const beginWidth = centerWidth - (VIDEO_PIXELS / 2);
    const beginHeight = centerHeight - (VIDEO_PIXELS / 2);
  ...
  onChangeSetting() {
    this.modalCtrl.create({ component: SettingsComponent, componentProps: { screenStatus: this.screenStatus } })
      .then(modalEl => {
  ...
  async onViewModels() {
    this.loadingCtrl.create({ message: 'Loading...' }).then(async (loadingEl) => {
